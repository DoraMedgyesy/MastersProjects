...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        still_unfinished_parses = []
        for p in unfinished_parses:
            if not (len(p.stack) == 1 and len(p.buffer) == 0):
                still_unfinished_parses.append(p)

        unfinished_parses = still_unfinished_parses
